...
public class OrderController {
  ...
  public ResponseEntity<OrderModel> order(@PathVariable UUID id, @RequestBody OrderDTO orderDTO) {
    final Optional<Order> originalOrder = service.findById(id);
    final Order saved = originalOrder
      .map(order -> {
  ...
  public ResponseEntity<OrderModel> order(@PathVariable UUID id) {
    Order order = service.findById(id).orElseThrow(() -> new OrderException(ORDER_NOT_FOUND, id));
